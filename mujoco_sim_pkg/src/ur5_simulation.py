...
class UR5Simulator:
    ...
    def get_joint_pos(self):
        ...
        joint_poisitons = np.asarray(self.data.qpos, dtype=np.float64)
        return joint_poisitons
    
    def get_joint_vels(self):

        joint_vels = np.asarray(self.data.qvel, dtype=np.float64)
        return joint_vels
    
    def get_joint_torques(self):

        joint_torques = np.asarray(self.data.qfrc_actuator, dtype=np.float64)
        return joint_torques
    
    def control(self, velocities):
...
if __name__ == "__main__":
    ...
    #     time.sleep(0.1)
    sim.data.qpos[:] = np.array([0.229509, -2.5, -1.6, 1.0, 1.5, 0])
    time.sleep(0.2)
